...
function casinoInfoEmbed() {
  return new EmbedBuilder()
    .setColor("Gold")
    .setTitle("🎰 Forzush Casino")
    .setDescription(
      [
        "ברוכים הבאים לקזינו של **Forzush Bot**.",
        "",
    ...
    .setFooter({
      text: "Forzush Casino • Virtual XP only"
    })
    .setTimestamp();
}
  ...
  return {
    embeds: [
      new EmbedBuilder()
        ...
        .setFooter({
          text: "Forzush XP Shop • Virtual XP only"
        })
        .setTimestamp()
    ],
    components: rows
  ...
  try {
    ...
    try {
      await member.roles.add(
        role,
        `Forzush XP Shop purchase: ${item.name}`
      );
    } catch (error) {
      console.error(
...
async function applyAutomaticWarnPunishment(guild, member) {
  ...
  await member.timeout(
    ...
    `Forzush automatic punishment: ${count} warns`
  );
    ...
    if (timer.type === "voice-mute") {
      if (
        ...
        await member.voice.setMute(
          false,
          "Forzush automatic Voice Unmute"
        ).catch(() => {});
      }
    }
...
function buildRankEmbed(guild, member) {
  ...
  return new EmbedBuilder()
    ...
    .setFooter({
      text:
        `Forzush • Staff Rank • ${member.user.username}`
    })
    .setTimestamp();
}
...
function buildStaffExamEmbeds() {
  const intro = new EmbedBuilder()
    ...
    .setDescription(
      [
        "ברוך הבא לבחינה לצוות של **Forzush**.",
        "",
    ...
    .setFooter({
      text: "Forzush • Staff Recruitment"
    })
    .setTimestamp();
  ...
  const partTwo = new EmbedBuilder()
    ...
    .setFooter({
      text: "Forzush Staff Team • Good Luck"
    });
...
function parseTicketTopic(channel) {
  ...
  if (!topic.startsWith("forzush-ticket:")) {
    return null;
  }
...
async function setTicketTopic(channel, data) {
  await channel.setTopic(
    `forzush-ticket:${data.ownerId}:${data.type}:${data.claimedBy || "none"}`
  );
}
...
async function openTicket(interaction, type) {
  ...
  const existing = interaction.guild.channels.cache.find(
    channel =>
      ...
      String(channel.topic || "").startsWith(
        `forzush-ticket:${interaction.user.id}:`
      )
  );
  ...
  const channel = await interaction.guild.channels.create({
    ...
    topic:
      `forzush-ticket:${interaction.user.id}:${type}:none`,
...
client.once(Events.ClientReady, async readyClient => {
  console.log(
    `✅ Forzush Bot online as ${readyClient.user.tag}`
  );
      ...
      if (command === "leaderboard" || command === "lb") {
        ...
        return message.reply({
          embeds: [
            new EmbedBuilder()
              .setColor("Gold")
              .setTitle("🏆 Forzush XP Leaderboard")
              .setDescription(
                top.map(
                  ([userId, data], index) =>
  ...
  async interaction => {
    try {
      ...
      if (interaction.commandName === "mute") {
        ...
        await member.voice.setMute(
          true,
          `${reason} | Forzush Voice Mute by ${interaction.user.tag}`
        );
      ...
      if (interaction.commandName === "unvoice-mute") {
        ...
        await member.voice.setMute(
          false,
          `Forzush Voice Unmute by ${interaction.user.tag}`
        );
      ...
      if (interaction.commandName === "chat-mute") {
        ...
        await member.roles.add(
          role,
          `${reason} | Forzush Chat Mute by ${interaction.user.tag}`
        );
      ...
      if (interaction.commandName === "un-chat-mute") {
        ...
        await member.roles.remove(
          role,
          `Forzush Chat Unmute by ${interaction.user.tag}`
        ).catch(() => {});
      ...
      if (interaction.commandName === "timeout") {
        ...
        await member.timeout(
          duration,
          `${reason} | Forzush by ${interaction.user.tag}`
        );
      ...
      if (interaction.commandName === "untimeout") {
        ...
        await member.timeout(
          null,
          `Forzush UnTimeout by ${interaction.user.tag}`
        );
